...
namespace Dota2ServerPingCheck
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            ServerStatus.OnUpdateData += RefreshGridEvent;
            ServerStatus.OnConsoleWrite += ConsoleWriteEvent;
        }
        ...
        //                server._packetLoss = 100 - ((server.TotalPacketsSent / server.TotalPacketsReceived) * 100);
        //                server.TotalPacketsSent = 0;
        //                server.TotalRoundTripTime = 0;
        //                server.TotalPacketsReceived = 0;
        //            }
        //        }
        //        else
        //        {
        ...
        //                   + "Total Packets Received = " + server.TotalPacketsReceived + "\r\n"
        ...
        private void ConsoleWriteEvent(object sender, EventArgs args)
        {
            object[] arr = (object[])sender;
            ConsoleWrite((string)arr[0] , (Color)arr[1]);
        }
        ...
        private void RefreshGrid()
        {
            ...
            {
                dataGrid.Invoke(new DelegateRefreshData(RefreshGrid), new object[] { });
            }
            else
            {
                dataGrid.Update();
        ...
        private void RefreshGridEvent(object sender, EventArgs args)
        {
            RefreshGrid();
        ...
        private void dataGrid_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Right)
            {
                ...
                if (currentMouseOverRow >= 0)
                {
                    ...
                    itemRefresh.Click += (sender2, e2) => ContextMenuItemRefresh(server);
                    m.MenuItems.Add(itemRefresh);
                    MenuItem itemDetail = new MenuItem(string.Format("Detail Refresh ping"));
                    itemDetail.Click += (sender2, e2) => ContextMenuItemDetailRefresh(server);
        ...
        private void ContextMenuItemRefresh(Dota2Server server)
        {
            ConsoleWrite("Requesting ping " + server.Name, Color.Lime);
            Application.UseWaitCursor = true;
            server.RefreshPing(false);
        }

        private void ContextMenuItemDetailRefresh(Dota2Server server)
        {
            ConsoleWrite("Requesting ping " + server.Name, Color.Lime);
            Application.UseWaitCursor = true;
            server.RefreshPing(true);
        }
        #endregion
